...
const Sidebar = () => {
          ...
          <div className="flex item-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-slate-500 truncate">
                {role === "ADMIN" ? "AADMINISTRATOR" : "EMPLOYEE"}
              </p>
            </div>
          </div>
        </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              to={item.href}
              className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-mediu, transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}
            >
              {isActive && (
  ...
  return (
    <div>
      ...
      <aside className="hidden lg:flex flex-col h-full w-[260px] bg-gradient-to-b from-slate-900 via-slate-900 to-slate-900 text-white shrink-0 border-r border-white/10">
        {sidebarContent}
      </aside>
